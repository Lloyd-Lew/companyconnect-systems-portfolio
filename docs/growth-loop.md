...
```mermaid
flowchart LR
    A[1. Capture buyer and client signals] --> B[2. Prioritise one high-value question or bottleneck]
    B --> C[3. Build or document original evidence]
    C --> D{Privacy and accuracy gate}
    D -- Needs client permission or redaction --> E[Sanitise, anonymise, or hold]
    E --> D
    D -- Approved --> F[4. Publish a focused website page or case study]
    F --> G[5. Create a concise GitHub portfolio summary]
    G --> H[6. Link relevant service, package, and next action]
    H --> I[7. Distribute through relevant channels]
    I --> J[8. Measure visibility, quality, and conversion]
    J --> K[9. Improve content, offer language, and internal process]
    K --> A

    subgraph Evidence_Standards[Evidence standards]
        L[First-hand delivery experience]
        M[Clear entities, dates, scope, and outcomes]
        N[No customer credentials, configurations, or confidential data]
    end
